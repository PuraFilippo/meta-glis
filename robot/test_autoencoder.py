...
from plot_utils import plot_outs
...
def test_model(test_loader, model_path, input_dim, latent_dim, device):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            encoded = autoencoder.encoder(xopt_pred)
            for i in range(encoded.size(2)):
                enc_flat = encoded[:, :, i].cpu().numpy().flatten()
                print(f'Latent variable {i}: ['+str(min(enc_flat))+', '+str(max(enc_flat))+']')
